...
class GGAN:
    ...
    def build_model(self, data, batch_size):
        ...
        # learning_rate = tf.train.inverse_time_decay(0.9, global_step, \
                                        # 1, 1)
        D_solver = tf.train.GradientDescentOptimizer(1.1).minimize(D_loss,
                                                        var_list=self.theta_D)
        G_solver = tf.train.GradientDescentOptimizer(1.1).minimize(G_loss,
        ...
        for it in range(nIters):
            ...
            _, D_loss_curr, d_w1, d_w2 = sess.run([D_solver, D_loss, self.D_W1, self.D_W2],
                    feed_dict={self.X: X_mb,
                                self.Z: self.sample_Z(batch_size, self.dim_z)})
            _, G_loss_curr, g_w1, g_w2 = sess.run([G_solver, G_loss, self.G_W1, self.G_W2],
                    feed_dict={self.Z: self.sample_Z(batch_size, self.dim_z)})


            # if it % print_counter == 0:
            #     d_w1_flat = np.reshape(d_w1, -1)
            #     d_w2_flat = np.reshape(d_w2, -1)
            #     g_w1_flat = np.reshape(g_w1, -1)
            #     g_w2_flat = np.reshape(g_w2, -1)
            #
            #     samples = [d_w1_flat, d_w2_flat, g_w1_flat, g_w2_flat]
            #     fig = plt.figure(figsize=(4, 4))
